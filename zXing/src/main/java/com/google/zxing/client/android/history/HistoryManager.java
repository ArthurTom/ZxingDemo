...
public final class HistoryManager {
    ...
    private final boolean enableHistory;    // 是否开启了存储到了数据库中

    public HistoryManager(Activity activity) {
        this.activity = activity;
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(activity);
        enableHistory = prefs.getBoolean(PreferencesActivity.KEY_ENABLE_HISTORY, true);  // 获取是否存储到历史目录中
    }
    ...
    /**
     * @param number 获取历史记录的位置
     * @return 获取的历史记录的集体的内容
     */
    ...
    public void addHistoryItem(Result result, ResultHandler handler) {
        ...
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(activity);
        // 是否打开了seting中保存重复记录的方法
        if (!prefs.getBoolean(PreferencesActivity.KEY_REMEMBER_DUPLICATES, false)) {
            deletePrevious(result.getText());   // 在数据库中删除重复的内容
        }
    ...
    /**
     * 功能：删除超过数据量超过2000后的数据
     */
    public void trimHistory() {
        SQLiteOpenHelper helper = new DBHelper(activity);  //初始化数据库
        try (SQLiteDatabase db = helper.getWritableDatabase();    // 获取数据的读写操作
             Cursor cursor = db.query(DBHelper.TABLE_NAME,        // 在数据库中中查找
                     ID_COL_PROJECTION,
                     null, null, null, null,
                     DBHelper.TIMESTAMP_COL + " DESC")) {
            cursor.move(MAX_ITEMS);   // 将指针移动到2000位置
            while (cursor.moveToNext()) {  //如果2000后还有内容，则将2000后的内容都在数据中删除
